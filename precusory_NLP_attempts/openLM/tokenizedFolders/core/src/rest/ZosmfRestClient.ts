 ...
 * / 
 ...
 export class ZosmfRestClient extends RestClient { 
     ...
     protected processError(original : IImperativeError ): IImperativeError { 
         ...
         try { 
             // do n't try to parse an empty string 
             if ( causeErrorsString ! = = " " ) { 
                 causeErrorsJson = JSON.parse(causeErrorsString ) ; 
                 // if we did n't get an error trying to parse causeErrorsString , check if there is a stack 
                 ...
                 // if we did n't get an error , make the parsed causeErrorsString part of the error 
