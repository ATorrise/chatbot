 ...
 * / 
 ...
 export class ImperativeConfig { 
     ...
     / * * 
      * This is the daemon context needed to pass to ` yargs.fail ( ) ` in the event that we can not extract 
      ...
      * 
      * It is n't hardcoded so that the name of our package can change without affecting 
     ...
     public getCallerFile(file : string ): any { 
         ...
         try { 
             ...
             // could n't locate using find - up , try to require directly 
             findupErr = e ; 
         } 
         // if we could n't find the file path through find - up , try requiring the string directly 
         try { 
             return require(file ) ; 
         } catch ( e ) { 
