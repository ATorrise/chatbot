 ...
 * / 
 ...
 const boolEnv = TEST_PREFIX + " _ OPT_FAKE_BOOL_OPT " ; 
 const stringEnv = TEST_PREFIX + " _ OPT_FAKE_STRING_OPT " ; 
 const stringAliasEnv = TEST_PREFIX + " _ OPT_FAKE_STRING_OPT_WITH_ALIASES " ; 
 const numberEnv = TEST_PREFIX + " _ OPT_FAKE_NUMBER_OPT " ; 
 const arrayEnv = TEST_PREFIX + " _ OPT_FAKE_ARRAY_OPT " ; 
 ...
 describe("CliUtils " , ( ) = > { 
     describe("extractEnvForOptions " , ( ) = > { 
         ...
         it("should not transform the env var string to a base 10 number if the value is n't a number ( results in a syntax error later ) " , ( ) = > { 
     ...
     describe("setOptionValue " , ( ) = > { 
         ...
         it("should return just the one property if it can not be converted to camel / kebab " , ( ) = > { 
