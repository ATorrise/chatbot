 ...
 describe("Config API tests " , ( ) = > { 
     ...
     describe("profiles " , ( ) = > { 
         describe("set " , ( ) = > { 
             ...
             it("should set a second level profile to a first level profile that does n't exist " , async ( ) = > { 
                 const config = await Config.load(MY_APP ) ; 
                 const profilesApi = config.api.profiles ; 
                 const fakeProfile : IConfigProfile = { 
                     type : " vegetable " , 
         ...
         describe("get " , ( ) = > { 
             ...
             it("should fail to get a profile that does n't exist " , async ( ) = > { 
                 const config = await Config.load(MY_APP ) ; 
                 const profile = config.api.profiles.get("vegetable " ) ; 
                 expect(profile).toEqual(null ) ; 
             } ) ; 
             it("should return empty object if optional profile does n't exist " , async ( ) = > { 
                 const config = await Config.load(MY_APP ) ; 
     ...
     describe("layers " , ( ) = > { 
         ...
         describe("activate " , ( ) = > { 
             const filePathProjectConfig = path.join(__dirname , " _ _ resources _ _ " , " project.config.json " ) ; 
             const filePathProjectUserConfig = path.join(__dirname , " _ _ resources _ _ " , " project.config.user.json " ) ; 
             const filePathAppConfig = path.join(__dirname , " _ _ resources _ _ " , " my_app.config.json " ) ; 
             const filePathAppUserConfig = path.join(__dirname , " _ _ resources _ _ " , " my_app.config.user.json " ) ; 
             beforeEach ( ( ) = > { 
                 jest.restoreAllMocks ( ) ; 
             ...
             it("should activate empty configuration in directory where it does n't exist " , async ( ) = > { 
                 const config = await Config.load(MY_APP ) ; 
                 jest.spyOn(path , " join").mockRestore ( ) ; 
                 const readLayerSpy = jest.spyOn(config.api.layers , " read " ) ; 
                 config.api.layers.activate(false , false , _ _ dirname ) ; 
                 const properties = config.api.layers.get ( ) ; 
                 expect(properties.user).toBe(false ) ; 
                 expect(properties.global).toBe(false ) ; 
             ...
             it("should activate configuration in current directory without reloading it " , async ( ) = > { 
                 ...
                 jest.spyOn(path , " join").mockRestore ( ) ; 
                 const readLayerSpy = jest.spyOn(config.api.layers , " read " ) ; 
                 config.api.layers.activate(false , false , path.join(__dirname , " _ _ resources _ _ " ) ) ; 
                 const properties = config.api.layers.get ( ) ; 
                 expect(properties.user).toBe(false ) ; 
                 expect(properties.global).toBe(false ) ; 
