 ...
 * / 
 ...
 describe("TeamConfig ProfileInfo tests " , ( ) = > { 
     ...
     const testDir = path.join(__dirname , " _ _ resources _ _ " ) ; 
     const teamProjDir = path.join(testDir , testAppNm + " _ team_config_proj " ) ; 
     const userTeamProjDir = path.join(testDir , testAppNm + " _ user_and_team_config_proj " ) ; 
     const teamHomeProjDir = path.join(testDir , testAppNm + " _ home_team_config_proj " ) ; 
     const largeTeamProjDir = path.join(testDir , testAppNm + " _ large_team_config_proj " ) ; 
     const nestedTeamProjDir = path.join(testDir , testAppNm + " _ nested_team_config_proj " ) ; 
     let origDir : string ; 

     const envHost = testEnvPrefix + " _ OPT_HOST " ; 
     const envPort = testEnvPrefix + " _ OPT_PORT " ; 
     const envRFH = testEnvPrefix + " _ OPT_RESPONSE_FORMAT_HEADER " ; 
     const envArray = testEnvPrefix + " _ OPT_LIST " ; 
     ...
     beforeEach ( ( ) = > { 
         ...
         process.env[testEnvPrefix + " _ CLI_HOME " ] = teamProjDir ; 
     } ) ; 
     ...
     describe("mergeArgsForProfile " , ( ) = > { 
         ...
         it("should throw if property location can not be found in JSON " , async ( ) = > { 
             const profInfo = createNewProfInfo(teamProjDir ) ; 
             await profInfo.readProfilesFromDisk ( ) ; 
             let caughtError ; 
         ...
         it("should not look for secure properties in the global - layer base profile if it does not exist " , async ( ) = > { 
             process.env[testEnvPrefix + " _ CLI_HOME " ] = nestedTeamProjDir ; 
     ...
     describe("updateProperty " , ( ) = > { 
         ...
         it("should attempt to store session config properties without adding profile types to the loadedConfig " , async ( ) = > { 
             ...
             const storageSpy = jest.spyOn(ConfigAutoStore as any , " _ storeSessCfgProps").mockResolvedValue(undefined ) ; 
     ...
     describe("updateKnownProperty " , ( ) = > { 
         ...
         it("should resolve to false if the property location can not be determined " , async ( ) = > { 
             const profInfo = createNewProfInfo(teamProjDir ) ; 
             await profInfo.readProfilesFromDisk ( ) ; 
