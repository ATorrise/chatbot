 ...
 * / 
 ...
 export class CommandUtils { 
     ...
     public static reconstructCommand(commandArguments : Arguments , 
         ...
         for ( const option of Object.keys(commandArguments ) ) { 
             if ( CommandUtils.optionWasSpecified(option , commandDefinition , commandArguments ) ) { 
                 // do n't print " true " for boolean options 
      ...
      * @param _ result ( recursion ) resulting list of flattened descendants 
      * @param _ tree ( recursion ) Initial tree definition 
     ...
     private static addChildAndDescendantsToSearch(prefix : string , child : ICommandDefinition , includeAliases : boolean = false , 
         _ result : ICommandTreeEntry [ ] = [ ] , _ tree : ICommandDefinition = child ) { 
         _ result.push ( { 
         ...
         for ( const descendant of child.children || [ ] ) { 
             CommandUtils.addChildAndDescendantsToSearch(prefix + child.name + " " , descendant , includeAliases , _ result , _ tree ) ; 
             if ( includeAliases ) { 
                 for ( const alias of child.aliases || [ ] ) { 
                     CommandUtils.addChildAndDescendantsToSearch(prefix + alias + " " , descendant , true , _ result , _ tree ) ; 
                 } 
             } 
         } 
         return _ result ; 
     ...
     public static getFullCommandName(commandDef : ICommandDefinition , commandTree : ICommandDefinition ): string { 
         ...
         // otherwise , could n't find it , just return the current name 
