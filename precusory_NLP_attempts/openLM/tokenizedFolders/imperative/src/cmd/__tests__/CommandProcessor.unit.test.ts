 ...
 * / 
 ...
 const SAMPLE_COMMAND_REAL_HANDLER : ICommandDefinition = { 
     ...
     description : " The banana command " , 
     type : " command " , 
     handler : _ _ dirname + " /__model__/TestCmdHandler " 
 } ; 
 ...
 const SAMPLE_COMMAND_REAL_HANDLER_WITH_OPT : ICommandDefinition = { 
     ...
     description : " The banana command " , 
     type : " command " , 
     handler : _ _ dirname + " /__model__/TestArgHandler " , 
     options : [ 
         { 
             name : " boolean - opt " , 
             type : " boolean " , 
 ...
 describe("Command Processor " , ( ) = > { 
     ...
     it("should just use the primary command ( if it can not infer the rest of the command ) in the syntax help message " , async ( ) = > { 
         // Allocate the command processor 
         const processor : CommandProcessor = new CommandProcessor ( { 
             envVariablePrefix : ENV_VAR_PREFIX , 
             fullDefinition : SAMPLE_COMPLEX_COMMAND , 
     ...
     it("should handle not being able to instantiate the handler " , async ( ) = > { 
         ...
         expect(commandResponse.error?.additionalDetails).toEqual ( 
             " Can not find module ' not_a_real_handler ' from ' packages / imperative / src / cmd / src / CommandProcessor.ts ' " 
         ) ; 
     } ) ; 
     ...
     it("should handle an error thrown from the handler " , async ( ) = > { 
         ...
         expect(commandResponse.message).toMatch ( 
             /Unexpected Command Error : Can not read ( property ' does nt ' of undefined|properties of undefined \(reading ' doesnt'\))/ 
         ) ; 
         expect(commandResponse?.error?.msg).toMatch(/Cannot read ( property ' does nt ' of undefined|properties of undefined \(reading ' doesnt'\))/ ) ; 
         ...
         expect(commandResponse?.error?.stack).toMatch ( 
             /TypeError : Can not read ( property ' does nt ' of undefined|properties of undefined \(reading ' doesnt'\))/ 
         ) ; 
     } ) ; 
     ...
     it("should invoke the handler and process daemon response and use the environment file " , async ( ) = > { 
         ...
         jest.spyOn(EnvFileUtils , " getEnvironmentFilePath").mockReturnValueOnce(join(__dirname , " _ _ resources _ _ " , " .zowe.env.json " ) ) ; 
     ...
     describe("invalidSyntaxNotification " , ( ) = > { 
         ...
         it("should log an invalid syntax notification with examples " , ( ) = > { 
             ...
             // will just check that the syntax generated has n't changed . 
