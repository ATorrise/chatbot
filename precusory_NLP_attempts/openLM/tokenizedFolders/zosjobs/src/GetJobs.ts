 ...
 * / 
 ...
 export class GetJobs { 
     ...
     / * * 
      * Get a single job object from an input job i d 
      * @static 
      * @param { AbstractSession } session - z / OSMF connection info 
     ...
     public static async getJob(session : AbstractSession , jobid : string ): Promise < IJob > { 
         ...
         const errorMessagePrefix : string = " Obtaining job info for a single job i d " + jobid + " on " + 
             session.ISession.hostname + " : " + session.ISession.port + " failed : " ; 

         const userMsg : string = " Can not obtain job info for job i d = " + jobid ; 
         ...
         // fail if no jobs 
         if ( jobs.length = = = 0 ) { 
             // TODO : V3_ERR_FORMAT - Do n't test for env variable in V3 
         ...
         // fail if unexpected number of jobs ( job i d should be unique ) 
         if ( jobs.length > 1 ) { 
             // TODO : V3_ERR_FORMAT - Do n't test for env variable in V3 
             if ( NextVerFeatures.useV3ErrFormat ( ) ) { 
                 throw new ImperativeError ( { 
      ...
      * @param jobid - the job i d for the job containing the spool content 
      * @param spoolId - i d number assigned by zosmf that identifies the particular job spool file ( DD ) 
      * @returns { Promise < string > } - promise that resolves to the spool content 
      * @memberof GetJobs 
      * / 
