use napi::{Env , Error , JsBoolean , JsUnknown , Result , Task } ; 
 ...
 impl Task for GetPassword { 
     ...
     fn reject(&mut self , _ env : Env , err : Error ) - > Result < Self::JsValue > { 
         Err(err ) 
     } 
 } 
 ...
 impl Task for SetPassword { 
     ...
     fn resolve(&mut self , env : Env , _ output : Self::Output ) - > Result < Self::JsValue > { 
 ...
 impl Task for FindCredentials { 
     ...
     fn resolve(&mut self , _ env : Env , output : Self::Output ) - > Result < Self::JsValue > { 
